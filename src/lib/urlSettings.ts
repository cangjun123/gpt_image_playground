...
const URL_SETTING_KEYS = ['settings', 'apiUrl', 'apiKey', 'codexCli', 'apiMode', 'model', 'imageGenerationStreaming']

function getProfileDedupKey(profile: Pick<AppSettings['profiles'][number], 'provider' | 'baseUrl' | 'apiKey' | 'model' | 'apiMode' | 'imageGenerationStreaming'>) {
...
export function buildSettingsFromUrlParams(currentSettings: Partial<AppSettings> | unknown, searchParams: URLSearchParams): Partial<AppSettings> {
  ...
  const imageGenerationStreamingParam = searchParams.get('imageGenerationStreaming')
  const apiMode: ApiMode | undefined = apiModeParam === 'images' || apiModeParam === 'responses' ? apiModeParam : undefined

  const hasLegacyOpenAIParams = apiUrlParam !== null || apiKeyParam !== null || codexCliParam !== null || apiMode !== undefined || modelParam !== null || imageGenerationStreamingParam !== null
  ...
  if (hasLegacyOpenAIParams) {
    ...
    if (imageGenerationStreamingParam !== null) profile.imageGenerationStreaming = imageGenerationStreamingParam.trim().toLowerCase() === 'true'
